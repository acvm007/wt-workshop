...
export default class {
  ...
  async buildConv(input, nameArray, relu6 = true, options = undefined) {
    ...
    if (nameArray[0].includes('depthwise')) {
      prefix += `FeatureExtractor_MobilenetV1_MobilenetV1_Conv2d_${nameArray[1]}_depthwise_BatchNorm_batchnorm`;
...
${nameArray[0]}_Conv2d_${nameArray[1]}_BatchNorm_batchnorm`;
        ...
        prefix += `FeatureExtractor_MobilenetV1_MobilenetV1_Conv2d_${nameArray[1]}_pointwise_BatchNorm_batchnorm`;
      }
      ...
      prefix += `FeatureExtractor_MobilenetV1_MobilenetV1_Conv2d_${nameArray[1]}_BatchNorm_batchnorm`;
    }

    const weightsName = prefix + weightSuffix;
    const weights = await buildConstantByNpy(this.builder, weightsName);
    const biasName = prefix.replace('nchw','nhwc') + biasSuffix;
